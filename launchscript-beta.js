console.log('Launching... launch script v 3.5.0');
...
async function loadGames() {
    ...
    const gameIDsFetch = await fetch("https://president-of-the-math-team.com/games-testing.json", { method: 'GET', cache: 'no-store' }).then(res => res.json()).catch(err => console.error(err));
    const gamesVersion = gameIDsFetch.version;
    const gameIDs = gameIDsFetch.list;

    const clientHtml = await fetch("https://president-of-the-math-team.com/client/v350.html").then(res => res.text()).catch(err => console.error(err));
    ...
    gameIDs.forEach(game => {
        game.elementId = currentElementID;
        currentElementID++;
    });
    ...
    gameIDs.forEach(game => {
        ...
        if (game.releaseDate) {
            ...
            let weekFromRelease = new Date(releaseDate).setDate(releaseDate.getDate() + 2);

            if (weekFromRelease > new Date()) {
                newGame = true;
            }

            // <div class="game-card-new">NEW!</div>
        }
    ...
    if (day == 1 || day == 2) {
        ...
        document.getElementById("weeklyUpdate").style.visibility = "visibile";
...
function createGameCard(name, id, saveid, image, url, description, alert, newGame) {
    ...
    gamecard.innerHTML = `${newGame}<p>${name}${description}</p><button onclick="${alert}updateURL('${url}');"><i class="fa-solid fa-arrow-up-right-from-square"></i> Launch</button><button onclick='favoriteScript(this);'><i class="fa-solid fa-heart grayed"></i><t>Favorite</t></button>`;
    return gamecard;
}
